...
namespace as
{
	...
	string get_file_prefix(string filename, bool cut_dot = false)
	{
		if (cut_dot)
	...
	//考虑难度，这里只写出符号本次使用的代码
	string stringf(const char *fmt, unsigned long a, unsigned long b, unsigned long c)
	{
		char temp[256] = {0};
		sprintf(temp, fmt, a, b, c);
		return string(temp);
	}
	...
	string guess_file_extension(unsigned char *buff, unsigned long len)
	{
