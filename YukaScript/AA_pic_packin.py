...
for f in fn:
    ...
    if file_name in mydic:
        ...
        #ykg package
        if flag == 1:
            png_offset1, png_length1, png_offset2, png_length2 = GetPngInfo(dst, mydic[file_name][0])
            ...
            if cur_size > org_size:
                ...
                #补0x00
                if zeros > 0:
                    dst.write(struct.pack('B', 0)*zeros)
                print(f)
        else:
            org_size = mydic[file_name][1]
            png_offset = mydic[file_name][0]
            if cur_size > org_size:
                print('\n\nFuck,Current File is larger than before!')
                print(f)
                input('Press Enter to continue')
