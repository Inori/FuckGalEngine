...
def makestr(lines):
    ...
    for index,line in enumerate(lines):
        if re.match('○[0-9A-Fa-f]+○', line):
            if  '★' in line:
                s = line.index('★')
                newstr_list.append(line[s+1:-1])
            else:
                newstr_list.append(line[10:-1])
        if re.match('●[0-9A-Fa-f]+●', line):
            if  '★' in line:
                s = line.index('★')
                oldstr_list.append(line[s+1:-1])
            else:
                oldstr_list.append(line[10:-1])

        #oldlen = len(oldstr_list)
        #newlen = len(newstr_list)
        #if (newlen - oldlen != 0) and (newlen - oldlen != 1):
        #    input('runtime number not match')
...
def WriteString(src, oldstr_list, newstr_list):
    ...
    for oldstr, newstr in zip(oldstr_list, newstr_list):
        ...
        newoff = src.tell()
        bstring = newstr.encode('sjis', errors = 'ignore')
        newlen = len(bstring)
        src.write(bstring)
