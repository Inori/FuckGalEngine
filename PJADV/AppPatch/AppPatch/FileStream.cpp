...
bool CFileStream::Open(char* pszFilePath)
{
	DWORD fileSize;
	DWORD readSize;
	fileHandle = CreateFileA(pszFilePath,GENERIC_READ,NULL,NULL,OPEN_EXISTING,FILE_ATTRIBUTE_NORMAL,NULL);
...
{
	fileHandle = CreateFileA(pszFilePath,GENERIC_READ|GENERIC_WRITE,FILE_SHARE_READ|FILE_SHARE_WRITE,NULL,CREATE_ALWAYS,FILE_ATTRIBUTE_NORMAL,NULL);
	if(fileHandle != INVALID_HANDLE_VALUE)
	{
		fileType = FileCreate;
...
bool CFileStream::AddFile(char* pszFilePath,char* treepath)
{
	...
	if(FileIsExists(treepath))
		return false;
	addFileHandle = CreateFileA(pszFilePath,GENERIC_READ,FILE_SHARE_READ|FILE_SHARE_WRITE,NULL,OPEN_EXISTING,FILE_ATTRIBUTE_NORMAL,NULL);
...
bool CFileStream::SaveToStream(BOOL bCompress)
{
	...
	if(bCompress)
	{
		...
		{
			MessageBoxA(NULL,"Ñ¹ËõÊ§°Ü","",MB_OK);
			abort();
		}
